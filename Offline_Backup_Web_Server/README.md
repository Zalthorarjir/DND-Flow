# DND Flow - Offline Backup Web Server
...
Offline mode is intended for local-only use, so Caddy is optional.

1. For normal offline use, simply run `start.bat` and open `http://localhost:5002`.
2. If you specifically want to test reverse-proxy / TLS locally, install Caddy separately from the official releases and either put `caddy.exe` in `tools/caddy/` locally or make `caddy` available in your system `PATH`.
3. Copy `Caddyfile.example` to `Caddyfile` and set your hostname only if you really need that local proxy setup.

The Caddy binary is intentionally not bundled in the repository so GitHub uploads stay below file-size limits.
